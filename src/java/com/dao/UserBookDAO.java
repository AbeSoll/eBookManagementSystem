...
public class UserBookDAO {
    ...
    public List<BookDetail> newBook() {
        ...
        BookDetail bd;
        try {
            String query = "SELECT * FROM book WHERE bookCategory = 'new' ORDER BY bookId DESC FETCH FIRST 8 ROWS ONLY";
    ...
    public List<BookDetail> oldBook() {
        ...
        BookDetail bd;
        try {
            String query = "SELECT * FROM book WHERE bookCategory = 'old' ORDER BY bookId DESC FETCH FIRST 4 ROWS ONLY";
            PreparedStatement pt = conn.prepareStatement(query);
            ResultSet rs = pt.executeQuery();
            while (rs.next()) {
                bd = new BookDetail();
    ...
    public List<BookDetail> newBookByPageNo(int pageNo) {
        ...
        BookDetail bd;
        try {
            String query = "SELECT * FROM book WHERE bookCategory = 'new' OFFSET " + ((pageNo - 1) * 20) + " ROWS FETCH NEXT 20 ROWS ONLY";
            PreparedStatement pt = conn.prepareStatement(query);
            ResultSet rs = pt.executeQuery();
            while (rs.next()) {
                bd = new BookDetail();
    ...
    public List<BookDetail> oldBookByPageNo(int pageNo) {
        ...
        BookDetail bd;
        try {
            String query = "SELECT * FROM book WHERE bookCategory = 'old' OFFSET " + ((pageNo - 1) * 20) + " ROWS FETCH NEXT 20 ROWS ONLY";
            PreparedStatement pt = conn.prepareStatement(query);
            ResultSet rs = pt.executeQuery();
            while (rs.next()) {
                bd = new BookDetail();
